...
defmodule Mix.Tasks.Pleroma.UserTest do
  ...
  describe "search" do
    test "it returns users matching" do
      ...
      assert [moon.id, kawen.id] == User.search("moon") |> Enum.map(& &1.id)
      res = User.search("moo") |> Enum.map(& &1.id)
      assert moon.id in res
      assert moot.id in res
      assert kawen.id in res
      assert [moon.id, kawen.id] == User.search("moon fediverse") |> Enum.map(& &1.id)

      assert [kawen.id, moon.id] ==
               User.search("moon fediverse", false, user) |> Enum.map(& &1.id)
    end
  end
end
